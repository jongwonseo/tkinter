...
# 파일 추가
def add_file():
  files = filedialog.askopenfilenames(title='이미지 파일을 선택하세요', filetypes=(("PNG 파일",'*.png'), ("모든 파일", '*.*')), initialdir=r'C:\Users\JDoubleU\Desktop\학교 자료\dummy')
...
btn_del_file = Button(file_frame, padx=5,  pady=5, width=12, text="파일추가", command=del_file)
...
root.resizable(False, False) #x,y값 변경 불가(크기 변경 불가)
root.mainloop()